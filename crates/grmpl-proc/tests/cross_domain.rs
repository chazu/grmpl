...
use grmpl_core::{
    Authority, DomainId, Entity, Fact, Message, Patch, RelId, Scope, TraceStore,
    Tuple, Value,
};
...
fn domain_a<'a>(store: &'a dyn TraceStore, tx: &'a dyn grmpl_core::Transport) -> Domain<'a> {
...
fn domain_b<'a>(store: &'a dyn TraceStore, tx: &'a dyn grmpl_core::Transport) -> Domain<'a> {
    Domain {
        id: B,
        store,
        transport: tx,
...
fn remote_emit_is_outboxed_shipped_and_applied_once() {
    ...
    // Two authority domains, each its own world — but the *same* substrate, or
    // the law would be comparing an ent against an LSM instead of testing either.
    let sib = c.sibling();
    let (store_a, store_b) = (c.store(), sib.store());
