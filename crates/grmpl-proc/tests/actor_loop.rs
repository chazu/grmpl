...
use grmpl_core::{
    Authority, DomainId, Entity, Fact, Patch, RelId, Result, Scope, TraceStore,
    Tuple, Value,
};
...
fn crash_after_commit_processes_each_message_exactly_once() {
    grmpl_conformance::for_each_world(|w| {
        {
            // Dropping the handle at the end of this block is the crash.
            let store = w.open();
