...
fn activations(store: &FjallStore) -> Vec<(i64, i64, i64)> {
...
fn cursor(store: &FjallStore) -> Option<Edition> {
    let at = store.current();
...
fn view_now(store: &FjallStore) -> BTreeMap<i64, i64> {
...
/// view_now)`.
fn run_scenario(seed: u64) -> (Activations, ViewSet) {
    let (_d, store) = store();
    let ow = on_watch();
    ow.install_including_current(&store, &NoSchemas).unwrap();
...
fn pump_delivery_holds_under_random_churn() {
    for seed in 0..32u64 {
        let (acts, view_now) = run_scenario(seed);
