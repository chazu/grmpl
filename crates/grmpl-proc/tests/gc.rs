...
fn consolidating_past_a_cursor_trips_the_edition_door() {
    grmpl_conformance::for_each_store(|c| {
        ...
        let sib_store2 = c.sibling();
        let store2 = sib_store2.store();
        churn(store2, 1, 0, 1);
        churn(store2, 2, 0, 1);
...
fn consolidate_to_clamps_to_min_cursor_under_random_churn() {
    grmpl_conformance::for_each_store(|c| {
        for seed in 1..=24u64 {
            // A fresh store of the same substrate each round — the expectation
            // below assumes a prior watermark of 0, and the watermark is
            // monotonic, so reusing one store across seeds would carry it over.
            let sib = c.sibling();
            let store = sib.store();
