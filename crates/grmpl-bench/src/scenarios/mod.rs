...
//!
//! Every scenario runs against a real [`EntStore`] in a fresh temp directory —
...
pub fn open_store() -> (TempDir, EntStore) {
    let dir = tempfile::tempdir().expect("tempdir");
    let store = EntStore::open(dir.path()).expect("open store");
    (dir, store)
}
