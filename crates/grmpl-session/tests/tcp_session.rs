...
fn a_client_builds_and_takes_over_tcp() {
    grmpl_conformance::for_each_store(|c| {
        ...
        assert_eq!(c.cmd("create orb", 1), vec!["Created orb."]);
        assert_eq!(c.cmd("take orb", 1), vec!["Taken."]);

        // The world really changed under the socket layer.
        let store = server.store();
        let held = store
            .read_at(server.world().relations().held, store.current())
            .unwrap();
        assert_eq!(
            held.len(),
            1,
            "the builder holds exactly one thing over TCP"
        );
    });
}
...
fn two_tcp_clients_racing_the_lamp_yield_one_winner() {
    grmpl_conformance::for_each_store(|c| {
        ...
        assert_eq!(builder.cmd("create orb", 1), vec!["Created orb."]);
